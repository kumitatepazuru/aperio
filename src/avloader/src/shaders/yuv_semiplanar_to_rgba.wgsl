...
// UV → Rg8Unorm texture (chroma half-size; R=U/Cb, G=V/Cr)

@group(0) @binding(0) var y_tex:  texture_2d<f32>;
@group(0) @binding(1) var uv_tex: texture_2d<f32>;  // Rg8Unorm: r=Cb, g=Cr
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let uv_raw = textureSample(uv_tex, yuv_sampler, in.uv).rg;  // Cb, Cr

    let y  = (y_raw       - 16.0  / 255.0) * (255.0 / 219.0);
    let cb = (uv_raw.r    - 128.0 / 255.0) * (255.0 / 112.0);
    let cr = (uv_raw.g    - 128.0 / 255.0) * (255.0 / 112.0);
