...
export function oklchToColorValue(oklchStr: string): ColorValue {
  try {
    const c = new CjsColor(oklchStr)
      .to("srgb")
      .toGamut({ space: "srgb", method: "clip" });
    const n = (x: number) => (isNaN(x) ? 0 : Math.max(0, Math.min(1, x)));
    return [n(c.coords[0] ?? 0), n(c.coords[1] ?? 0), n(c.coords[2] ?? 0), 1];
  } catch {
    return [0, 0, 0, 1];
  }
}
...
export const BLACK: ColorValue = [0, 0, 0, 1];
export const WHITE: ColorValue = [1, 1, 1, 1];
