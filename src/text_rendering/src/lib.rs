...
impl TextRenderer {
    ...
    fn collect_char_glyph_bounds(&mut self, spec: &TextSpec) -> Vec<(char, u32, u32, u32, u32)> {
        ...
        if let Some(ref fam) = spec.font_family {
            ...
            buffer.set_text(&mut self.font_system, &spec.text, &attrs, Shaping::Advanced, None);
        } else {
            buffer.set_text(&mut self.font_system, &spec.text, &Attrs::new(), Shaping::Advanced, None);
        }
    ...
    fn render_impl(&mut self, spec: &TextSpec) -> Result<Option<Arc<wgpu::Texture>>> {
        ...
        let page_draws: Vec<PageDraw> = page_order
            ...
            .map(|&page| {
                ...
                self.queue.write_buffer(&instance_buf, 0, bytemuck::cast_slice(insts));
