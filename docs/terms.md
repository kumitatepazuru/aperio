...
| 例                          | 意味                                       |
| --------------------------- | ------------------------------------------ |
| `"base_effect.test_object"` | base プラグインの test_object オブジェクト |
| `"base_effect.blur_effect"` | base プラグインの blur エフェクト          |
...
- Python プラグインでは `self.name = "base_effect.blur_effect"` のように設定し、`register_sub_plugin()` で登録される。
